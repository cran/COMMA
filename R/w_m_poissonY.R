...
#'
#' @return \code{w_m_poissonY} returns a matrix of E-step weights for the EM-algorithm.
...
w_m_poissonY <- function(mstar_matrix, outcome_matrix,
                        pistar_matrix, pi_matrix,
                        p_yi_m0, p_yi_m1,
                        sample_size, n_cat){
  
  y_m1_mstar1_frac1 = p_yi_m1 * mstar_matrix[,1] *
    pistar_matrix[1:sample_size, 1] * pi_matrix[,1]
  y_m2_mstar1_frac1 = p_yi_m0 * mstar_matrix[,1] *
  ...
  y_mstar1_frac2 = (p_yi_m1 * pistar_matrix[1:sample_size, 1] * pi_matrix[,1]) +
    (p_yi_m0 * pistar_matrix[1:sample_size, 2] * pi_matrix[,2])
  
  y_mstar2_frac2 = (p_yi_m0 *
                    pistar_matrix[(sample_size + 1):(n_cat * sample_size), 2]
                  * pi_matrix[,2]) +
    (p_yi_m1 * pistar_matrix[(sample_size + 1):(n_cat * sample_size), 1] * pi_matrix[,1])
  
  
  m1_term = (y_m1_mstar1_frac1 / y_mstar1_frac2) + (y_m1_mstar2_frac1 / y_mstar2_frac2)
  m2_term = (y_m2_mstar1_frac1 / y_mstar1_frac2) + (y_m2_mstar2_frac1 / y_mstar2_frac2)
